...
During a login or a payment session the user consent to share information if it's requested by the merchant. This information is accessible for the merchant for the next 168 hours, even though the user revokes the consents in this period. 

The reason we do it like this is because; 
* The user in theory can revoke the consent both during the payment- and login-session which can case problems for some merchant.
* Some merchants needs up till 168 hours to fetch user info. 
